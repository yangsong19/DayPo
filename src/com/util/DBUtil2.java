...
public class DBUtil2 {
	private static final Logger logger = Logger.getLogger(DBUtil2.class);

	// 打开数据库连接（type: MySQL，Oracle，SQLServer）
	public static Connection openConnection(String type, String host,
	...
	public static <T> T queryBean(QueryRunner runner, Class<T> cls,
		...
		try {
			if (MapUtil.isNotEmpty(map)) {
				result = runner.query(sql, new BeanHandler<T>(cls,
	...
	public static <T> List<T> queryBeanList(QueryRunner runner, Class<T> cls,
		...
		try {
			if (MapUtil.isNotEmpty(map)) {
				result = runner.query(sql, new BeanListHandler<T>(cls,
						new BasicRowProcessor(new BeanProcessor(map))), params);
			} else {
	...
	public static Object queryColumn(QueryRunner runner, String column,
		...
		Object result = null;
		try {
			result = runner.query(sql, new ScalarHandler<Object>(column),
					params);
		} catch (SQLException e) {
			logger.error(e.getMessage(), e);
			throw new RuntimeException(e.getMessage(), e);
	...
	public static <T> List<T> queryColumnList(QueryRunner runner,
		...
		try {
			result = runner
					.query(sql, new ColumnListHandler<T>(column), params);
		} catch (SQLException e) {
			logger.error(e.getMessage(), e);
			throw new RuntimeException(e.getMessage(), e);
		}
